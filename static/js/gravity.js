...
function stepRocket(gameState, constants) {
  ...
  var dvxAccelerometer =
    constants.accelerometerFactor * gameState.accelerometer[0] * constants.dt;
  var dvyAccelerometer =
    constants.accelerometerFactor * gameState.accelerometer[1] * constants.dt;
  vx += dvxAccelerometer;
  vy += dvyAccelerometer;
  ...
  if (!gameState.renderingOrbit && gameState.highlightPath.length > 4) {
    ...
    var leastDistance = Math.hypot(lastPoint, secondLastPoint);
    ...
    gameState.highlightPath
      .slice(0, -20)
      .forEach(function (innerPoint, innerPointIndex) {
        var distInnerLast = Math.hypot(lastPoint, innerPoint);
    ...
    if (
      ...
      if (closestPointNeighborIndex <= lastIndex) {
        var closestPointNeighbor =
          gameState.highlightPath[closestPointNeighborIndex];
...
function renderForceVectors(graphics, constants) {
  ...
  for (var j = 0; j < numYVectors; j++) {
    for (var i = 0; i < numXVectors; i++) {
      ...
      xEndCanvas =
        xCanvas + dvxNormalized * cellRadiusCanvas * vectorLengthFudgeFactor;
      yEndCanvas =
        yCanvas + dvyNormalized * cellRadiusCanvas * vectorLengthFudgeFactor;
      graphics.ctx.beginPath();
...
function renderGame(gameState, constants, graphics) {
  ...
  var ptXCanvas =
    gameState.highlightPath[numPathPts - 1][0] * graphics.canvasWidth;
  var ptYCanvas =
    (1.0 - gameState.highlightPath[numPathPts - 1][1]) * graphics.canvasHeight;
  graphics.ctx.beginPath();
  ...
  for (var i = 0; i < pathLength; i++) {
    ...
    ptYCanvas =
      (1.0 - gameState.highlightPath[indexFromEnd][1]) * graphics.canvasHeight;
